<div class="container">
		<div class="col-md-12 post">
			<h1>Личный кабинет</h1>
				<?PHP 
					...
					$Q_CHBIZZ = mysqli_query($CONNECT, "SELECT * FROM `businesses` WHERE `bOwner` = '$_SESSION[USER_LOGIN]' LIMIT 3");
					$Q_CHFARM = mysqli_query($CONNECT, "SELECT * FROM `farms` WHERE `owner` = '$_SESSION[USER_LOGIN]' LIMIT 1");
					$CHHOUSE = mysqli_fetch_array(mysqli_query($CONNECT, "SELECT * FROM `house` WHERE `hOwner` = '$_SESSION[USER_LOGIN]' LIMIT 1"));
					$AccFamily = mysqli_fetch_array(mysqli_query($CONNECT, "SELECT * FROM `s_family` WHERE `fID` = '$Row[pFamily]'"));
					...
					echo '<table style="text-align: left; width: 100%; height: auto;" border="0">
					<tbody>
						<tr>
							<td width="30%">
								<center>
									<h3>'.str_replace("_", " ", $Row['Name']).'</h3>
									<img src="../resource/img/skins/'.$SKIN[0].'.png" /><br>
								</center>
							</td>
							<td width="50%">
								Номер аккаунта&nbsp; <strong>'.$Row['pID'].'</strong><br>
								Уровень&nbsp; <strong>'.$Row['pLevel'].'</strong><br>
								Очки опыта&nbsp; <strong>'.$Row['pExp'].'/'.$NEXT_LEVEL.'</strong><br><br>
								Наличные&nbsp; <strong>$'. $Row['pCash'].'</strong><br>
								Деньги в банке&nbsp; <strong>$'. $Row['pBank'].'</strong><br>
								На депозите&nbsp; <strong>$'. $Row['pDeposit'].'</strong><br><br>
								Номер телефона&nbsp; <strong>'.$Row['pPnumber'].'</strong><br>
								E-mail&nbsp; <strong>'.$Row['pEmail'].'</strong><br>
								Фишек с казино&nbsp; <strong>'.$Row['pCasinoChips'].' шт.</strong><br><br>
								На сотрудничестве&nbsp; <strong>'.$pmedia.'</strong><br>
							</td>';
							UCP_MENU();
						echo '</tr>
					  </tbody>
					...
					echo '<br><div class="ucrinf">
						...
						<div>
							...
							<p class="inf">';
								...
								else echo '<b>У Вас отсутствует дом!</b>';
							echo '</p>
					...
					<center><h3><strong>У Вас отсутствуют бизнесы!</strong></h3></center></div> <br><div class="ucrinf">';
						if($NUM_FARM > 0){
					...
					<center><h3><strong>У вас отсутствует личный транспорт!</strong></h3></center></div>';
				?>
			<br>
			<br>
		</div>
	...
	style="display:block"
	data-ad-client="ca-pub-3836052749840469"
	data-ad-slot="1256102771"
	data-ad-format="auto"
